...
//   const userData = await sql.dbConn("select user,userpassword from users where username='" + username + "userPassword" + password, "'", "development_usercred")
...
server.post("/login", async (request, response) => {
  ...
  try {
    ...
    if (user.length !== 0) {
      ...
      response.send({ message: "The username you've entered is incorrect" });
    }
  } catch (error) {
